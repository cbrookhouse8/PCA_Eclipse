...
public class Ellipsoid {
	...
	public Ellipsoid(PApplet p, Matrix valsVecs, Matrix vcs) {
		this.p = p;
		Matrix pcsExtents = new Matrix(new float[] { 0, 0, 0, 0, 0, 0 }, 3, 2);
		/*
		 * | min_dot_pc1 max_dot_pc1 | | min_dot_pc2 max_dot_pc2 | | min_dot_pc3
		 * max_dot_pc3 |
		 */

		pc1ExtentVecs = new Matrix(3, 2);
		/*
		 * | pc1_low_X pc1_high_X | | pc1_low_Y pc1_high_Y | | pc1_low_Z pc1_high_Z |
		 */
	...
	public void display(Matrix transforms, Matrix display) {
		...
		// p.stroke("#52AAF5");
		p.noFill();
		p.ellipse(calculatedQ.M[0], calculatedQ.M[1], 20, 20);
		p.fill(255);
		p.text("cQ", calculatedQ.M[0], calculatedQ.M[1]);
		...
		for (int k = 0; k < steps; k++) {
			...
			for (int j = 0; j < steps; j++) {
				...
				p.vertex(surfacePoint.M[0], surfacePoint.M[1]);
			}
			p.endShape(p.CLOSE);
		}
